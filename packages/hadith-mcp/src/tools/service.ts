import { collectionAliases, fixtureCollections, fixtureProvenanceNotes, fixtureRecords } from "../fixtures/records.js";
import type { CollectionSummary, HadithRecord, Language, SearchResult, ToolError } from "../types.js";

const importVersion = "fixture-0";

function canonicalizeCollection(collection: string): string | null {
  return collectionAliases.get(collection.trim().toLowerCase().replace(/\s+/g, "-")) ?? null;
}

function normalizeHadithNumber(hadithNumber: string | number): string {
...
export function listCollections(): { collections: CollectionSummary[] } {
  return { collections: fixtureCollections() };
}

export function fetchHadith(args: {
  collection: string;
...
export function searchHadith(args: {
  query: string;
...
export function validateHadithReference(args: {
  collection: string;
  hadith_number: string | number;
}): {
  valid: boolean;
...
export function getHadithMetadata(args: {
  collection: string;
  hadith_number: string | number;
}): {
  result:
...
export function getHadithGrade(args: {
  collection: string;
  hadith_number: string | number;
}): {
  result:
